...
- In **prospective sampling** or **cohort study**, the predictors are fixed and then the outcome is observed. 
...
$$
\frac{p_j(\mathbf{x}_{0j})}{\sum_{i=0}^M p_j(\mathbf{x}_{0j})} =  \frac{\exp \mathbf{x}_{0j}^T \boldsymbol{\beta}}{\sum_{i=0}^M \exp \mathbf{x}_{ij}^T \boldsymbol{\beta}}.
$$
