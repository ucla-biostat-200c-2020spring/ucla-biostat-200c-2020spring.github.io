...
- **Exercise**: Do similar binned plots for deviance residuals against `weights` and deviance residuals vs `cigs` to check the linearity assumption. 

- QQ plot is not helpful since there is no reason these deviance residuals are approximately standard normal.
```{r}
qqnorm(devres)
```
Half-normal plot can help detect unusual cases in predictor space. For logistic regression, we use the generalized hat matrix
$$
...
wcgs_binned <- wcgs %>%
  ...
  group_by(bin) %>%
  summarize(y = sum(ifelse(chd == "yes", 1, 0)), avgpred = mean(predprob), count = n()) %>%
...
nrow(wcgs_binned)
# p-value
pchisq(hlstat, nrow(wcgs_binned), lower.tail = FALSE)
```
...
- If we choose a threshold, say 0.25, then the predicted probabilities give a classification rule
$$
\text{case $i$ is a} \begin{cases}
\text{"success"} & \text{if } \widehat{p}_i \ge 0.25 \\
\text{"failure"} & \text{if } \widehat{p}_i < 0.25
\end{cases}.
$$
```{r}
wcgs %>% 
  mutate(predprob = predict(lmod, type = "response")) %>% 
  mutate(predout = ifelse(predprob >= 0.25, "yes", "no")) %>%
...
The **sensitivity** is
$$
\frac{\text{TP}}{\text{TP + FN}} = \frac{0}{257} = 0\%
$$
and the **specificity** is
$$
\frac{\text{TN}}{\text{FP + TN}} = \frac{2894}{3 + 2894} = 99.9\%
$$
...
(train_data <- wcgs[training_samples, ])
(val_data <- wcgs[-training_samples, ])
```
...
x_train <- model.matrix(
  ...
  data = train_data)
x_train %>% head(10)
y_train <- ifelse(train_data$chd == "yes", 1, 0)
y_train %>% head(10)
```
...
- Now we can evaluate performance of the models (corresponding to different $\lambda$ values) on the validation set.
```{r}
# form y and x from validation data
x_val <- model.matrix(
  chd ~ - 1 + age + height + weight + bmi + sdp + dbp + chol + dibep + cigs + arcus, 
  data = val_data)
y_val <- ifelse(val_data$chd == "yes", 1, 0)
...
```
From the graph, it seems that $\lambda = 0.05$ yields a model that performs best on the validation set.  
...
plot(cv_lasso)
```
The plot displays the cross-validation error (devience by default) according to the log of lambda. The left dashed vertical line indicates that the log of the optimal value of lambda is approximately -6.4, which is the one that minimizes the prediction error. This lambda value will give the most accurate model. The exact value of lambda and corresponding model can be viewed as follow:
```{r}
cv_lasso$lambda.min
