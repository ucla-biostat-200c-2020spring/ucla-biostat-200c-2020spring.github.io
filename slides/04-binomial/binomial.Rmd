...
- If we detect a lack of fit (unusual large deviance or Pearson $X^2$), we consider following causes.
    ...
    3. Sparse data. If binomial batch sizes are all small, then the $\chi^2$ approximation is bad. Rule of thumb for using the $\chi^2$ approximation is $n_i \ge 5$ for all batches.
...
- Violation of the iid (identically and independently distributed) assumption of Bernoulli trials. (HW2: compare the variance of beta-binomial to that of binomial.)
...
- $U_i$ acts like the derivative of a log-likelihood. Thus we define
$$
