...
- When $Y$ is truely binomial and $m_i$ are relatively large, the deviance $D$ is approximately $\chi^2$ distributed with $n - q - 1$ degrees of freedom if the model is correct. The large p-value indicates that the model has an adequate fit.
```{r}
pchisq(lmod$deviance, lmod$df.residual, lower = FALSE)
```
A rule of thumb to use this asymptotic approximation is $m_i \ge 5$.
...
\end{eqnarray*}
Comparing Pearson $X^2$ statistic to the asymptotic null distribution $\chi^2$ with degrees of freedom $n - q - 1$ gives a goodness of fit test.
```{r}
...
- Violation of the iid (identically and independently distributed) assumption of Bernoulli trials can lead to inflation of variance. (HW2: compare the variance of beta-binomial to that of binomial.)
...
$$
where $p$ is the number of parameters consumed by the logistic regression. Estimation of $\boldsymbol{\beta}$ is unchanged but the inference is changed
$$
...
- A deviance value of `r deviance(bmod)` on `r df.residual(bmod)` suggests a lack of fit. Diagnostics (not done here) does not reveal outliers. We estimate the dispersion parameter as
```{r}
...
\end{eqnarray*}
**HW2**: Verify that the log-likilihood $\ell_i$ of a binomial proportion $Y_i$, where $m_i Y_i \sim \text{Bin}(m_i, p_i)$, satisfies  
\begin{eqnarray*}
...
\end{eqnarray*}
with $\phi = 1$, $\mu_i = p_i$, and $V(\mu_i)=p_i (1 - p_i)/m_i$.
...
- The `mammalsleep` data records the sleep behavior of 62 mammals.
```{r}
