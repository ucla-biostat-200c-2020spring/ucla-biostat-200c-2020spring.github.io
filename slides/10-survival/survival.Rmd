...
wb_df %>%
  ggplot() + 
  geom_line(mapping = aes(x = y, y = S)) + 
  labs(x = "y", y = "f(y)", title = "Survivor functiion of Weibull")
```
...
wb_df %>%
  ggplot() + 
  geom_line(mapping = aes(x = y, y = h)) + 
  labs(x = "y", y = "f(y)", title = "Hazard functiion of Weibull")
```
...
summary(pmod)
```
Note the inference (standard errors) is for the Poisson model. Correct inference can be obtained from the information matrix of $(\lambda, \boldsymbol{\beta})$.

    **Exercise**: interpret the coefficient $\beta_{\text{treatment}} = -1.73$.
