...
- Income trajectories stratified by sex. We observe men's incomes are generally higher and less variable than women's. Women's income seems to increase quicker than men's. How to test these?
...
$$
$\sigma_e^2$ is interpreted as the variance for `eyes` for a given subject.
...
```{r}
mmod <- lmer(acuity ~ power + (1 | subject) + (1 | subject:eye), data = vision, REML = FALSE)
nmod <- lmer(acuity ~ (1 | subject) + (1 | subject:eye), data = vision, REML = FALSE)
KRmodcomp(mmod, nmod)
```

- If we exclude individual 6, then `power` becomes significant.

```{r}
mmodr <- lmer(acuity ~ power + (1 | subject) + (1 | subject:eye), data = vision, REML = FALSE, subset = -43)
nmodr <- lmer(acuity ~ (1 | subject) + (1 | subject:eye), data = vision, REML = FALSE, subset = -43)
KRmodcomp(mmodr, nmodr)
```

- To test significance of the variance component parameters,
