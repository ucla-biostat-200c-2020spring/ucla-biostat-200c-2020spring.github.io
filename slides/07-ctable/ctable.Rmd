...
![](./outlook-for-the-global-semi-conductor-silicon-wafer-industry.jpg)
...
$$
\sum_{i, j} \frac{(y_{ij} - \widehat \mu_{ij})^2}{\widehat \mu_{ij}},
$$
where
$$
...
- We observe the manufacturing process for a certain period of time and observe 450 wafers. The data are then cross-classified.
...
- If we assume the total sample size is fixed at $n=450$. Then we can model the counts by a multinomial model. We consdier two models  
    ...
    - $H_1$: one full/saturated model where each cell of the $2 \times 2$ has it's own probability $p_{ij}$, and 
...
- If we view `particle` as a predictor affecting whether a wafer is good quality or bad quality, we end up with a binomial model. Equivalently we consider the row sums as fixed.
...
- When there are more than rows or columns, this model is called the product binoimal/multinomial model.

### Hypergeometric

- If we fix both row and column marginal totals, the probability of the observed table is
\begin{eqnarray*}
...
- We follow up by a **correspondence analysis** to study how `eye` and `hair` are dependent. 
    ...
    1. We first compute the table of Pearson residuals $\mathbf{R}$.
    ```{r}
...
- The `femsoke` records female smokers and non-smokers, their age group, and whether the subjects were dead or still alive. 
```{r}
...
prop.table(ct, 1)
```
we see higher percentage of non-smokers died than smokers and it is significant
```{r}
summary(ct)
```

- However `smoke` status is confounded with the `age` group.
```{r}
prop.table(xtabs(y ~ smoker + age, data = femsmoke), 2)
```
Smokers are more concentrated in the younger groups and younger people are more likely to live for another 20 years.
...
- It leads to a model with with two interaction terms
```{r}
glm(y ~ smoker * age + age * dead, family = poisson, data = femsmoke) %>%
  summary()
```
...
```{r}
ctf <- xtabs(fitted(modu) ~ smoker + dead + age, data = femsmoke)
...
- We can start from the saturated model and use analysis deviance to see how much we reduce the model.
```{r}
glm(y ~ smoker * age * dead, family = poisson, data = femsmoke) %>%
  step(test = "Chi") %>%
  drop1(test = "Chi")
```
The interaction term `smoker:dead` is weakly significant in the final model. This corresponds to the test of conditional independence between smoking and life status given age group. The p-value is very similar to that from the CMH test.

### Binomial

- It makes intuitive sense to model life status as the outcome depending on perdictors `smoker` and `age`. 
