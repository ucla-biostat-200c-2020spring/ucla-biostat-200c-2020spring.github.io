...
The `gavote` data contains the voting data of GA in the 2000 presidential election. It is available as a dataframe. We convert it into a tibble for easy handling by tidyverse.
```{r}
gavote <- gavote %>% 
...
- Number of votes can be smaller than the number of ballots because a vote is not recorded if (1) the person fails to vote for President, (2) votes for more than one candidate, or (3) the equipment fails to record the vote. 

  We are interested in the `undercount`, which is defined as `(ballots - votes) / ballots`. Does it depend on the type of voting machine `equip`, economy `econ`, percentage of African Americans `perAA`, whether the county is rural or urban `rural`, or whether the county is part of Altanla metropolitan area `atlanta`.
...
- For factor `rural`, we found it has the same name as one level in this factor. To avoid confusion, let's rename it to `usage`.
...
Let's further stratify according to categorical variable `equip`, `econ`, `usage`, and `atlanta`.
```{r}
...
```{r}
library(GGally)
ggpairs(data = gavote, columns = c(3, 12, 13, 11), aes(color = econ)) + 
...
where   
    ...
    - the **predictors** $X_1,\ldots,X_{p-1}$ encodes the independent variables `perAA`, `econ`, etc.,   
...
$$
- It's important to keep in mind that the least squares estimate only assumes that the errors $\epsilon_i$ (1) are independent, (2) have mean 0 and (3) have equal variance $\sigma^2$. Under these assumptions, the celebrated Gauss-Markov theorem states that the least squares estimate is the best (smallest variance) linear unbiased estimates. 
...
- We start with a linear model with just predictors: percentage of Gore votes `pergore` and percentage of African Americans `perAA`
$$
...
```
Inspection of the result, stored in `lmod`, shows that it contains rich information. 
```{r}
str(lmod)
```
...
$$
\widehat{\boldsymbol{\epsilon}} = \mathbf{y} - \mathbf{X} \widehat{\boldsymbol{\beta}} = \mathbf{y} - \hat{\mathbf{y}}. 
$$
```{r}
...
$$
R^2 = 1 - \frac{\sum_i (y_i - \hat{y_i})^2}{\sum_i (y_i - \bar{y})^2} = 1 - \frac{\text{RSS}}{\text{TSS}},
$$
...
- **Exerciese**: Interpret regression coefficient.  
    - How do we interpret $\hat \beta_0 = 0.043$?   
    - How do we interpret $\hat \beta_{\text{cperAA}} = 0.0283$?    
    - How do we interpret $\hat \beta_{\text{equipOS-PC}} = 0.016$? 
    - How do we interpret $\hat \beta_{\text{usageurban}} = -0.019$? 
    - How do we interpret $\hat \beta_{\text{cpergore:usageurban}} = -0.009$? 
...
- We can carry out the similar $F$-test for each predictor in a model using the `drop1` function. The nice thing is that the factors such as `equip` and `cpergore * usage` are drooped as a group. Q: Why `usage` and `cpergore` are not tested?
```{r}
drop1(lmodi, test = "F")
```

    We also see $F$-test for quantitative variables, e.g., $cperAA$, conincides with the $t$-test reported by the `lm` function.

## Confidence intervals

- Confidence intervals for individual parameters can be construced based on their null distribution
$$
\frac{\widehat{\beta_j}}{\text{se}(\widehat{\beta_j})} \sim t_{n-p}.
$$
...
- Typical assumptions of linear models are
    ...
    2. Errors $\epsilon_i$ are independent and normally distributed with common variance $\sigma^2$.  
    We'd like check these assumptions using graphical or numerical approaches. 
...
$$
\mathbf{X} \widehat{\boldsymbol{\beta}} = \mathbf{X} (\mathbf{X}^T \mathbf{X})^{-1} \mathbf{X}^T \mathbf{y} = \mathbf{H} \mathbf{y}.
$$
...
$$
If $h_i$ is large, then $\text{var}(\hat{\epsilon_i}) = \sigma^2 (1 - h_i)$ is small. The fit is "forced" to be close to $y_i$. In this sense it is an **influential point**. Points on the boundary of the predictor space have the most leverage. 
...
$$
where $r_i$ are the standardized residuals and $\hat{y}_{(i)}$ are the predicted values if the $i$-th observation is dropped from data. A large residual combined with a large leverage results in
a larger Cook statistic.  

- Graphical diagnostics reveal a few potential **outliers**
```{r}
gavote %>% 
  slice(c(3, 9, 96, 120)) %>%
  print(width = Inf)
...
- In presence of outliers, if these outliers represent data entry errors, then we can remove these observations and proceed with linear regression. 

- If these outliers are realy observations, then we can use robust linear regression.
```{r}
library(MASS)
...
- In this case, **transformation of response** `undercount` is not desirable because 
...
Variable selection concerns the problem selecting the best set of variables to put in a model. There are several approaches and it is still an active research area. 
