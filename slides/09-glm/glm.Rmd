...
\end{eqnarray*}
**Show this in HW3**. For GLMs with canonical links, the second term and third term cancel using the fact
$$
\frac{d\mu_i}{d \eta_i} = \frac{d\mu_i}{d \theta_i} = \frac{d \, b'(\theta_i)}{d \theta_i} = b''(\theta_i) = \frac{\sigma_i^2}{a(\phi)}.
$$
It's immediate that the negative Hessian is positive semidefinte and Newton's algorithm with line search is stable. 
