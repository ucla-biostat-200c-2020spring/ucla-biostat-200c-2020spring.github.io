...
$$
where $f_j$ are smooth arbitrary functions. Interaction terms are ignored. To incorporate categorical predictors, we can use the model
$$
...
- `gam` package uses a **backfitting algorithm** for fitting the additive models.
    ...
    1. Initialize $\beta_0, \beta_1, \ldots, \beta_p$ say from the least squares fit.  
...
$$
for a family of spline basis function, $\phi_i$. The smoothing penalty $\int [f_j''(x)]^2 \, dx$ translates to the term $\boldsymbol{\beta}_j^T \mathbf{S}_j \boldsymbol{\beta}_j$ for a suitable $\mathbf{S}_j$ that depends on the choice of basis. It tha maximizes the penalized log-likelihood
$$
...
- The response is `O3` (ozone level). The predictors are `temp` (temperature at El Monte), `ibh` (inverse base height at LAX), and `ibt` (inversion top temperature at LAX). 
```{r}
