...
summary(lmod)
```
The contrast `contr.sum` forces the coefficients for 4 operators sum to 0. So the coeffiicient for the fourth operator is `r -sum(coef(lmod)[2:4])`.
```{r}
model.matrix(lmod)
```
...
- The hypothesis being test in the ANOVA is _are these four operators same_ in terms of the paper brightness they produce. This is called a **fixed effects model** because the effects $\alpha_i$ are assumed to be fixed. 

- Suppose we are interested in the research question _are the population of operators all same_ based on this data set? We will arrive to a random effects model
$$
y_{ij} = \mu + \alpha_i + \epsilon_{ij}, \quad i = 1,\ldots,4, j = 1,\ldots,5,
$$
where we assume
    ...
    This is called a **random effects** model because now the effects $\alpha_i$ are assumed to be random.
...
- Traditionally, linear models have been divided into three categories:
    ...
    3. Mixed effects model: $\mathbf{y} = \mathbf{X} \boldsymbol{\beta} + \mathbf{Z} \boldsymbol{\gamma} + \boldsymbol{\epsilon}$, where $\boldsymbol{\beta}$ is fixed and $\boldsymbol{\gamma}$ is random.
...
- Primary goal of the mixed model (aka variance components model) is to  
    ...
    - estimation and testing of the variance components,   $(\sigma^2, \sigma_1^2, \ldots, \sigma_m^2)$, and  
    - prediction.  

## Example: one-way ANOVA 

- For the one-way ANOVA random effects model
$$
y_{ij} = \mu + \alpha_i + \epsilon_{ij}, \quad i=1,\ldots,a
$$
we recognize it as a mixed effects model
\begin{eqnarray*}
\mathbf{y} = \mathbf{1}_n \beta_0 + \begin{pmatrix}
\mathbf{1}_{n_1} & & \\
& \vdots & \\
& & \mathbf{1}_{n_a}
\end{pmatrix} \boldsymbol{\gamma} + \boldsymbol{\epsilon},
\end{eqnarray*}
where $\boldsymbol{\gamma} \sim N(\mathbf{0}_a, \sigma_{\alpha}^2 \mathbf{I}_a)$ and $\boldsymbol{\epsilon} \sim N(\mathbf{0}_{\sum_i n_i}, \sigma_{\epsilon}^2 \mathbf{I})$ are independent. $n_i$ are number of observations in level $i$. 

- How do we estimate parameters $\beta_0$, $\sigma_{\alpha}^2$, and $\sigma_{\epsilon}^2$?
...
- The familar ANOVA table gives the partition
\begin{eqnarray*}
...
- Let's find the MLE for the `pulp` data using `lme4` package. The syntax `(1 | operator)` instructs that the data is grouped by `operator` and the `1` indicates that random effect is constant within each group. 
...
- Let $\mathbf{K} \in \mathbb{R}^{n-p}$ be any basis of the space ${\cal N}(\mathbf{X}^T)$. Then
$$
\mathbf{K}^T \mathbf{Y} \sim N(\mathbf{0}_{n-p}, \mathbf{K}^T (\mathbf{Z} \boldsymbol{\Sigma} \mathbf{Z}^T + \sigma^2 \mathbf{I}) \mathbf{K}).
$$
We estimate variance component parameters $(\sigma^2, \boldsymbol{\Sigma})$ by MLE using this transformed data. Then we estimate fixed effects $\boldsymbol{\beta}$ using general least squares.

- Let's find the REML for the `pulp` data.
```{r}
library(lme4)
...
- If we compare two nested models that differ only in fixed effects, we can use the standard likelihood ratio test (LRT). Remember we cannot use REML estimates for LRT.  

    F tests for fixed effects can fail. What degrees of freedom to use? 

- Testing the variance component parameters, e.g., $H_0:\sigma_{\alpha}^2=0$, can be hard too. Traditional method like LRT can be wrong. E.g., for the `pulp` data, LRT using the the conventional $\chi_1^2$ null distribution gives a non-significant p-value. 
```{r}
...
- The idea of parameteric bootstrap is we generate new $y$ from the fitted null model many times. For each simulation replicate, we carry out the LRT. Then the p-value is estimated by the proportion of simulation replicates that generate LRT test statistic larger than the observed one $2.5684$.
...
- Advantages of parametric boostrap:  
    - It's not restricted to MLE estimators. It applies to any other estimators.  
    - It does not rely on the dubious asymptotic $\chi_2$ distribution.  
...
\end{eqnarray*}
$$
The random effects $\boldsymbol{\gamma}$ are random variables. It does not make sense to estimate $\boldsymbol{\gamma}$. 

- However, if we can take Bayesian point of view. We have a **likelihood**
$$
\mathbf{y} \mid \boldsymbol{\gamma} \sim N(\mathbf{X} \boldsymbol{\beta}, \sigma^2 \mathbf{I}_n)
$$
and a **prior distribution**
$$
\boldsymbol{\gamma} \sim N(\mathbf{0}_a, \boldsymbol{\Sigma}).
$$
...
- We can use this posterior mean to restimate random effects. For the `pulp` data
```{r}
ranef(mmod)$operator
```
...
```
That's why Bayesian estimates are often called the shrinkage estimator.
...
- In the `penicillin` data set, we want to study how `yield` depends on the `treat` (proccess) and `blend` (corn steep liquor). 
...
- For this balanced case, we can use `aov` to test significance of fixed effect `treat`
```{r}
amod <- aov(yield ~ treat + Error(blend), data = penicillin)
summary(amod)
```

- The F test also applies to this balanced case.
```{r}
anova(mmod)
```

- Parametric bootstrap. We observed LRT statistic
```{r}
...
```
The dubious $\chi_3$ approximation gives p-value
```{r}
pchisq(4.0474, 3, lower.tail = FALSE)
```
Now we use parametric bootstrap
```{r}
set.seed(123)
B <- 1000
lrstat <- numeric(B)
...
mean(lrstat > 4.0474)
```
The `pbkrtest` packages automate this parametric boostrap procedure.
```{r}
library(pbkrtest)
...
for (i in 1:B) {
  ...
  lrstatf[i] <- 2 * (logLik(rmod) - logLik(nlmod, REML = TRUE))
}
mean(lrstatf > 2.7629)
```
