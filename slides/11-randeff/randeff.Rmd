...
\end{eqnarray*}
where $\boldsymbol{\gamma} \sim N(\mathbf{0}_a, \sigma_{\alpha}^2 \mathbf{I}_a)$ and $\boldsymbol{\epsilon} \sim N(\mathbf{0}_{\sum_i n_i}, \sigma_{\epsilon}^2 \mathbf{I})$ are independent. Note in $\mathbf{Z}$ we have one column for each level. 
...
for (i in 1:B) {
  ...
  lrstatf[i] <- 2 * (logLik(rmodr, REML = FALSE) - logLik(nlmodr, REML = FALSE))
}
