...
\end{eqnarray*}
where $\widehat{\mu}_i$ are the fitted values from the model. The Poisson deviance is also called the **G-statistic**. See HW2 for the justification of last equality.
...
\end{eqnarray*}
We shall use a Bernoulli response model for $f_1$ to model the probability of observing 0, and use the zero-truncated Poisson model for $f_2$ to model the probability of observing an outcome greater than 0. 
...
- The **zero-inflated Pisson** (ZIP) assumes a mixture model
\begin{eqnarray*}
...
- Prediction. For a single male with no children whos mentor procuded six articles, we predict that the chance he produces no articles is 0.278. 
```{r}
